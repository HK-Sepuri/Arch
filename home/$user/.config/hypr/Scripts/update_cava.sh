...
# Extract colors
color1=$(echo "$colors" | sed -n '16p' | tr -d '\n')
color2=$(echo "$colors" | sed -n '14p' | tr -d '\n')
color3=$(echo "$colors" | sed -n '12p' | tr -d '\n')
color4=$(echo "$colors" | sed -n '10p' | tr -d '\n')
color5=$(echo "$colors" | sed -n '8p' | tr -d '\n')
color6=$(echo "$colors" | sed -n '6p' | tr -d '\n')
color7=$(echo "$colors" | sed -n '4p' | tr -d '\n')
color8=$(echo "$colors" | sed -n '2p' | tr -d '\n')
